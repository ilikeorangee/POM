...
/**
 * Send request to start temporal search as a given time
 */
 int temporal_setTime(int sockfd, char *device_ID, char *timestamp, char *message) {
 	...
 	sprintf(
 		request,
		"{\"format\":%d, \"type\":%d, \"action\":%d, \"deviceID\":\"%s\", \"timestamp\":\"%s\"}",
		TEMPOREQUEST,
		NO_NODE,
		SET_TEMPORAL_TIME,
		device_ID,
 	...
 	n = write(sockfd, request, strlen(request));
 	if (n < 0)
 		_error("[temporal_setTime] failed writing to socket");

	_wait_for_reply(sockfd, response);
	sscanf(response, "{\"format\":%*d,\"type\":%*d,\"action\":%d,\"status\":%d,\"message\":\"%[^\"]\"}", &action, &status, message);
	if (status == 1) {
		sscanf(response, "{\"format\":%*d,\"type\":%*d,\"action\":%*d,\"status\":%*d,\"message\":\"%[^\"]\"}", req_error);
		return status;
	}
...
/**
 * Send request to retrieve the value of the specified element in temporal search
 */
int get_element_value_temporal(int sockfd, char *device_ID, char *element_ID, char *value) {
	...
	if (n < 0)
		_error("[get_element_temporal] failed writing to socket");
...
int get_attr_value_temporal(int sockfd, char *device_ID, char *element_ID, char *attr_name, char *value) {
	...
	if (n < 0)
		_error("[get_attr_value_temporal] failed writing to socket");
...
int locate_service_temporal(int sockfd, char *device_ID, char *element_ID, char *service) {
	...
	if (n < 0)
			_error("[locate_service_temporal] failed writing to socket");
...
/**
 * Sets up kqueue
 * Returns kqueue file descriptor
 */
int kqueue_setup() {
    int kfd;
